...
// https://github.com/danielgsims/php-collections/blob/2.X/docs/Collection.md

namespace Benchpress\Collections;

use ArrayIterator;
use Benchpress\Exceptions\InvalidArgumentException;
use Benchpress\Exceptions\OutOfRangeException;
...
class Collection implements ICollection {
     ...
     * {@inheritdoc}
     */
    public function add($item): ICollection|static {
     ...
     * {@inheritdoc}
     */
    public function clear(): ICollection|static {
     ...
     * {@inheritdoc}
     */
    public function contains(callable $condition): bool {
     ...
     * {@inheritdoc}
     */
    public function findIndex(callable $condition): int {
        $index = -1;
     ...
     * {@inheritdoc}
     */
    public function filter(callable $condition): ICollection|static {
        $items = [];
     ...
     * {@inheritdoc}
     */
    public function findLastIndex(callable $condition): int {
        $index = -1;
     ...
     * {@inheritdoc}
     */
    public function slice($start, $end): ICollection|static {
     ...
     * {@inheritdoc}
     */
    public function insert($index, $item): static {
        $this->validateIndex($index);
        $this->validateItem($item, $this->type);
     ...
     * {@inheritdoc}
     */
    public function insertRange($index, array $items): static {
        $this->validateIndex($index);
        $this->validateItems($items, $this->type);
     ...
     * {@inheritdoc}
     */
    public function without(callable $condition): Collection|ICollection|static {
     ...
     * {@inheritdoc}
     */
    public function removeAt($index): ICollection|static {
        $this->validateIndex($index);
        $items = $this->items;
     ...
     * @throws InvalidArgumentException
     */
    public function reverse(): ICollection|static {
     ...
     * {@inheritdoc}
     */
    public function sort(callable $callback): ICollection|static {
        $items = $this->items;
     ...
     * {@inheritdoc}
     */
    public function drop($num): Collection|ICollection|static {
     ...
     * {@inheritdoc}
     */
    public function dropRight($num): Collection|ICollection|static {
     ...
     * {@inheritdoc}
     */
    public function dropWhile(callable $condition): Collection|ICollection|static {
     ...
     * {@inheritdoc}
     */
    public function tail(): Collection|ICollection|static {
     ...
     * {@inheritdoc}
     */
    public function take($num): Collection|ICollection|static {
        if ($num > $this->count()) {
            $num = $this->count();
        }
     ...
     * {@inheritdoc}
     */
    public function takeRight($num): Collection|ICollection|static {
        if ($num > $this->count()) {
            $num = $this->count();
        }
     ...
     * @return int
     */
    protected function countWhileTrue(callable $condition): int {
        $count = 0;
     ...
     * {@inheritdoc}
     */
    public function takeWhile(callable $condition): Collection|ICollection|static {
        $count = $this->countWhileTrue($condition);
     ...
     * {@inheritdoc}
     */
    public function shuffle(): ICollection|static {
        $items = $this->items;
        shuffle($items);
     ...
     * {@inheritdoc}
     */
    public function merge($items): ICollection|static {
        if ($items instanceof ICollection) {
            $items = $items->toArray();
        }
    ...
    public function headAndTail(): array {
        if ($this->count() === 0) {
            return [ null, null ];
        }
