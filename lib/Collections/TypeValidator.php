...
trait TypeValidator
{
    ...
    private function nonScalarTypeExists($type): bool {
     ...
     */
    protected function validateItems(array $items, string $type): void {
        foreach ($items as $item) {
            $this->validateItem($item, $type);
        }
    }

    protected function validateItem(mixed $item, string $target): void {
        $type = gettype($item);
        ...
        if (!$looseObjectCheck && !$shouldBeCallable && $isObject && !is_a($item, $target)) {
